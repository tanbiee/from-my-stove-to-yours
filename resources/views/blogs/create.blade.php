...
<div style="max-width:780px; margin:0 auto; padding:1rem 0;">
    ...
    <div style="margin-bottom:2.5rem;">
        <p style="font-size:.75rem; letter-spacing:.2em; text-transform:uppercase; color:var(--gold); font-weight:600; margin-bottom:.5rem;">✦ Share Your Story</p>
        ...
        <p style="color:var(--warm-gray); font-size:.95rem; line-height:1.6;">
            Share your kitchen adventures, cultural food traditions, techniques and tales from around the world.
        </p>
    </div>
    ...
    <form action="{{ route('blogs.store') }}" method="POST"
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.2rem; margin-bottom:1.5rem;">
            <div>
                ...
                <div style="position:relative;">
                    <select name="category" id="category" class="field-input" style="appearance:none; cursor:pointer; padding-right:2.5rem;" required>
                        <option value="" disabled selected>Choose a cuisine or topic…</option>
                        @foreach($categories as $cat => $icon)
                            <option value="{{ $cat }}" @selected(old('category')===$cat)>{{ $icon }} {{ $cat }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="number" name="read_time_min" id="read_time_min" class="field-input"
                       min="1" max="60" placeholder="e.g., 8" required value="{{ old('read_time_min') }}">
        ...
        <div style="margin-bottom:1.5rem;">
            ...
            {{-- Preview --}}
            <div id="imgPreview" style="display:none; margin-top:.8rem; border-radius:.75rem; overflow:hidden; height:200px;">
        ...
        <div style="margin-bottom:1.5rem;">
            ...
            <textarea name="content" id="content" rows="14" class="field-input"
                      ...
                      placeholder="Take us on a journey — describe the dish, its origins, personal memories, cooking tips or the culture behind it…" required>{{ old('content') }}</textarea>
        ...
        <div style="display:flex; justify-content:flex-end; gap:1rem; padding-top:1.2rem; border-top:1px solid #ede6d8;">
            <a href="{{ route('blogs.index') }}" class="btn-outline">Cancel</a>
