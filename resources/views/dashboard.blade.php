                ...
                <article style="background:var(--card-bg); border-radius:1rem; overflow:hidden; border:1px solid var(--border-light);">
                    <a href="{{ route('recipes.show', $recipe->id) }}" style="text-decoration:none; color:inherit; display:block;">
                        <div style="height:180px; background:#f6f2ee; overflow:hidden; display:flex; align-items:center; justify-content:center;">
                            @if($recipe->image)
                                <img src="{{ asset('images/'.$recipe->image) }}" alt="{{ $recipe->title }}" style="width:100%; height:100%; object-fit:cover; display:block;" />
                            @else
                                <div style="padding:2rem; color:var(--warm-gray);">No image</div>
                            @endif
                        </div>
                        <div style="padding:1rem 1.25rem;">
                            ...
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span style="color:var(--warm-gray); font-size:.85rem;">Recipe #{{ $recipe->id }}</span>
                                <span style="color:var(--warm-gray); font-size:.85rem;"></span>
                            </div>
                        </div>
                    </a>
                </article>
